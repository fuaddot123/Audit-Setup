...
async function pastikanOpsi({ tabel, label, tambahan, daftar, userId }) {
  ...
  const { data: d2, error: e2 } = await supabase
    ...
    .single();
  if (e2)
    throw new Error(`Gagal menambah "${bersih}" ke daftar: ${e2.message}`);
  return d2.kode;
}
...
export async function hapusDisplayUntukPeriode({ branchId, period }) {
  ...
  const { data: kondisiPeriodeIni, error: kErr } = await supabase
    .from("display_kondisi")
    .select("id, display_unit_id, photos")
    .eq("period", period)
    .in("display_unit_id", unitIds);
  if (kErr) throw kErr;
  if (!kondisiPeriodeIni || !kondisiPeriodeIni.length) return; // nggak ada yang perlu dibersihin

  const unitIdsPeriodeIni = [
    ...new Set(kondisiPeriodeIni.map((k) => k.display_unit_id)),
  ];
  ...
  const { data: riwayatLain, error: rErr } = await supabase
    ...
    .in("display_unit_id", unitIdsPeriodeIni)
    .neq("period", period);
  if (rErr) throw rErr;
  const punyaRiwayatLain = new Set(
    (riwayatLain || []).map((r) => r.display_unit_id)
  );

  const idUnitBaruLahir = unitIdsPeriodeIni.filter(
    (id) => !punyaRiwayatLain.has(id)
  );
  ...
  const semuaFoto = kondisiPeriodeIni.flatMap((k) =>
    Array.isArray(k.photos) ? k.photos : []
  );
  if (semuaFoto.length) {
    const paths = semuaFoto
      .map((m) => {
  ...
  if (idUnitBaruLahir.length) {
    await supabase
      .from("display_kondisi")
      .delete()
      .in("display_unit_id", idUnitBaruLahir);
  ...
  const idUnitTurunBulanIni = (unitRows || [])
    .filter(
      (u) =>
        idUnitLama.includes(u.id) &&
        (u.tanggal_turun || "").slice(0, 7) === period
    )
    .map((u) => u.id);
  if (idUnitTurunBulanIni.length) {
    await supabase
      ...
      .update({
        tanggal_turun: null,
        perlakuan_kode: null,
        perlakuan_tanggal: null,
        perlakuan_catatan: null,
        harga_jual_display: null,
      })
...
export async function muatDisplay({ branchId, period }) {
  const [unitRes, perlakuanRes, kondisiOpsiRes] = await Promise.all([
    supabase
      .from("v_display_monitoring")
      .select("*")
      .eq("branch_id", branchId)
      .order("umur_hari", { ascending: false }),
    supabase
      .from("display_perlakuan")
      .select("*")
      .eq("aktif", true)
      .order("urutan"),
    supabase
      .from("display_kondisi_opsi")
      .select("*")
      .eq("aktif", true)
      .order("urutan"),
  ]);
  ...
  const kondisiByUnit = {};
  kondisiPeriode.forEach((k) => {
    kondisiByUnit[k.display_unit_id] = k;
  });
  ...
  const rows = relevan.map((u) => {
    ...
    return {
      ...
      perlakuan_catatan: "",
      harga_jual_display:
        u.harga_jual_display == null ? "" : String(u.harga_jual_display),
      baru: false,
    };
  });
...
export function periksaDisplay(rows) {
  ...
  rows.forEach((r, i) => {
    ...
    if (r.baru) {
      ...
      if (!r.tanggal_pajang)
        galat.push(`${nama}: tanggal mulai pajang belum diisi.`);
      if (r.program_brand && !r.program_nama.trim())
        galat.push(
          `${nama}: ditandai ikut program brand tapi nama programnya kosong.`
        );
    }
    if (r.turun && !r.perlakuan_kode)
      galat.push(
        `${nama}: ditandai turun display tapi perlakuannya belum dipilih.`
      );
    if (r.kondisi_kode === OPSI_BARU) {
      if (!r.kondisi_baru_label.trim())
        galat.push(
          `${nama}: kondisi baru dipilih tapi istilahnya belum ditulis.`
        );
      ...
      if (!r.kondisi_baru_setara)
        galat.push(
          `${nama}: kondisi baru "${r.kondisi_baru_label.trim()}" belum dipilih setara dengan tingkat apa.`
        );
    }
    if (
      r.turun &&
      r.perlakuan_kode === OPSI_BARU &&
      !r.perlakuan_baru_label.trim()
    )
      galat.push(
        `${nama}: perlakuan baru dipilih tapi istilahnya belum ditulis.`
      );
...
export async function simpanDisplay({ rows, branchId, period, auditDate, userId, kondisiOpsi, perlakuanOpsi, }) {
  ...
  for (const r of rows) {
    if (r.kondisi_kode === OPSI_BARU) {
      ...
      if (!petaKondisi[kunci]) {
        const setara = (kondisiOpsi || []).find(
          (k) => k.kode === r.kondisi_baru_setara
        );
        petaKondisi[kunci] = await pastikanOpsi({
          tabel: "display_kondisi_opsi",
          label: r.kondisi_baru_label,
    ...
    if (r.turun && r.perlakuan_kode === OPSI_BARU) {
      ...
      if (!petaPerlakuan[kunci]) {
        petaPerlakuan[kunci] = await pastikanOpsi({
          tabel: "display_perlakuan",
          label: r.perlakuan_baru_label,
          tambahan: {},
          daftar: perlakuanOpsi,
          userId,
        });
      }
    }
  ...
  const kodeKondisi = (r) =>
    r.kondisi_kode === OPSI_BARU
      ? petaKondisi[r.kondisi_baru_label.trim().toLowerCase()]
      : r.kondisi_kode;
  const kodePerlakuan = (r) =>
    r.perlakuan_kode === OPSI_BARU
      ? petaPerlakuan[r.perlakuan_baru_label.trim().toLowerCase()]
      : r.perlakuan_kode;
  ...
  for (const r of rows) {
    ...
    if (r.turun && r.masih_dipajang) {
      const { error } = await supabase
        ...
        .update({
          ...
          harga_jual_display:
            r.harga_jual_display === "" ? null : Number(r.harga_jual_display),
        })
        .eq("id", unitId);
      if (error) throw new Error(`${r.brand} ${r.model}: ${error.message}`);
    }
    ...
    if (r.kondisi_kode) {
      ...
      const opsiTerpakai = (kondisiOpsi || []).find(
        (k) => k.kode === kodeKondisi(r)
      );
      const skorBeku =
        r.kondisi_kode === OPSI_BARU
          ? (
      ...
      if (res.error)
        throw new Error(`${r.brand} ${r.model}: ${res.error.message}`);
    }
...
export async function uploadDisplayMedia({ branchId, period, idx, fileList }) {
  ...
  for (const file of files) {
    ...
    if (uploadFile.size > maxSize)
      throw new Error(
        `Ukuran ${isVideo ? "video" : "foto"} maksimal ${ isVideo ? "30MB" : "5MB" }.`
      );
