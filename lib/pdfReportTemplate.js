...
export function buildSummaryReportHtml(opts) {
  const {
    ...
    badgeCol, // opsional — index kolom yang dirender sebagai badge warna. Default: kolom terakhir
              // (perilaku lama, biar laporan lain yang belum pakai opsi ini nggak berubah).
  } = opts;
  ...
  const tbodyHtml = tableRows.map((r) => {
    const badgeIdx = badgeCol ?? r.cells.length - 1;
    const cells = r.cells.map((c, i) => {
      if (i === badgeIdx && r.badge) {
...
export function openPrintWindow(title, pagesHtml, opts = {}) {
  const win = window.open("", "_blank");
  if (!win) return false;
  // compactMode: dipake buat laporan yang ISINYA banyak elemen non-teks (foto, kartu, tabel)
  // dan nggak boleh font-nya ikut kekecilin pas coba muat di 1 halaman (misal Berita Acara).
  // Bedanya sama mode default (zoom): compactMode NGGAK pernah nyentuh font-size sama sekali —
  // yang dikecilin cuma padding/margin/gap/ukuran gambar lewat class .compact-1..compact-N,
  // yang class-nya (dan aturan CSS-nya) didefinisikan sendiri sama pemanggil di HTML-nya.
  // Kalau abis semua step compact masih kepanjangan juga, dibiarin lanjut ke halaman 2
  // (nggak dipaksa muat 1 halaman dengan cara apapun lagi) — sesuai constraint "jangan font".
  const compactMode = !!opts.compactMode;
  const compactSteps = opts.compactSteps || ["compact-1", "compact-2", "compact-3", "compact-4"];
  const fitScript = compactMode
    ? `
      function fitToPage() {
        const zoomEl = document.getElementById("pdfZoom");
        const targetHeight = 1000; // ruang aman cetak A4
        const steps = ${JSON.stringify(compactSteps)};
        let i = 0;
  ...
  win.document.write(`<!DOCTYPE html><html><head><title>${esc(title)}</title><meta charset="utf-8"><style>${REPORT_STYLE}</style></head><body><div id="pdfZoom">${pagesHtml}</div>
    <script>
      ${fitScript}
