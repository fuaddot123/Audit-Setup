...
// ============================================================
// Terjemahan manual "item lolos" -> "kalimat temuan" (dipakai Laporan
// Kunjungan). Banyak item ditulis sebagai pernyataan kondisi BAIK ("Tidak
// ada X", "Y sesuai ketentuan"), jadi kalau item itu GAGAL/nggak dicentang,
// nulis ulang teksnya APA ADANYA salah arti — dibaca kayak pencapaian,
// padahal maksudnya kebalikannya. Dipetakan MANUAL per item (bukan tebak
// regex) biar tiap kalimat tetap masuk akal secara bahasa, nggak ada yang
// keliru arah negasinya. Key: "catId|index" (index 0-based sesuai urutan
// items[] di CATS). Item yang nggak ada di sini (harusnya nggak ada,
// seluruh 75 item CATS baru dicakup) jatuh ke fallback di temuanText().
const NEGASI_TEMUAN = {
  "display_laptop|1": "Jumlah unit display tidak sesuai SOP (6-8 unit tergantung island)",
  "display_laptop|2": "Ada unit display yang dijual tanpa pricetag sesuai sistem",
  "display_laptop|5": "Kabel tidak rapi / berantakan",
  "display_laptop|6": "Ada unit display yang tidak ON sesuai ketentuan",
  "display_laptop|7": "Monitor/TV tidak menampilkan konten standard promosi",
  "display_laptop|8": "Area bawah meja tidak bersih dari sarang laba dan kotoran",
  "display_laptop|11": "Laptop putih tidak menggunakan wrapping putih",
  "display_laptop|12": "Karet bawah laptop tidak menggunakan solasi kertas",
  "display_laptop|14": "Ruangan tidak harum / bau",
  "display_aksesoris|1": "Ada unit aksesoris yang tidak berpricetag",
  "gudang|0": "Box tidak tersusun rapi",
  "gudang|4": "Lantai tidak bersih / bernoda",
  "gudang|5": "Rak tidak bersih / ada sarang laba-laba",
  "gudang|7": "Tidak ada tempat sampah kering",
  "gudang|9": "Penerimaan barang tidak maksimal 1x24 jam",
  "gudang|11": "Pengiriman surat jalan tidak sesuai ketentuan",
  "gudang|12": "Kardus bekas tidak tersusun di tempat terpisah",
  "kasir|0": "Meja kasir tidak bersih",
  "kasir|1": "Ada dokumen/nota berserakan / tidak terdistribusi dengan baik",
  "kasir|2": "Uang kas kecil tidak sesuai",
  "kasir|3": "Tidak ada tempat sampah kering",
  "kasir|4": "Ada transaksi yang tidak memiliki nota resmi",
  "kasir|5": "Nota diedit tanpa izin HO",
  "toilet|0": "Tidak ada tempat sampah",
  "toilet|1": "Lantai tidak bersih / bernoda parah / ada sampah berceceran di lantai/wastafel",
  "toilet|2": "Berbau tidak sedap",
  "pelayanan|1": "Tidak membukakan pintu customer",
  "pelayanan|2": "Tidak menyambut customer \u22645 detik",
  "pelayanan|4": "Tidak posisi berdiri standby",
  "service|0": "Ada unit service yang tidak tercatat di sistem",
  "service|3": "Tools tercecer",
  "service|5": "Tidak ada tempat sampah kering",
  "grooming|0": "Grooming hijab tidak sesuai standar",
  "grooming|1": "Grooming pria tidak sesuai standar",
  "grooming|2": "Tidak lengkap seragam KLA & celana gelap & lanyard dan id card",
  "grooming|3": "Berbau tidak sedap",
  "depan_toko|2": "Halaman tidak bersih",
  "depan_toko|3": "Lantai depan tidak bersih",
  "non_operasional|0": "Area lantai tidak bersih dari kotoran/kerak/noda",
  "non_operasional|2": "Area bawah tangga tidak bersih/rapi",
  "non_operasional|3": "Kunci toko tidak sesuai ketentuan",
};

// Ubah 1 teks item (yang GAGAL/nggak dicentang) jadi kalimat temuan yang
// masuk akal. Urutan coba: (1) map manual di atas — paling akurat; (2) item
// yang emang udah diawali "Tidak ada "/"Tidak " — tinggal dibalik/dilepas
// negasinya; (3) fallback tanda \u274c di depan teks asli (dipakai buat item
// checklist LAMA yang nggak ikut dipetakan manual, biar tetap kebaca ada
// masalah walau kalimatnya nggak diubah).
export function temuanText(catId, idx, original) {
  const manual = NEGASI_TEMUAN[`${catId}|${idx}`];
  if (manual) return manual;
  const t = (original || "").trim();
  if (/^tidak ada /i.test(t)) return t.replace(/^tidak ada /i, "Ada ");
  if (/^tidak /i.test(t)) return t.replace(/^tidak /i, "");
  return "\u274c " + t;
}
...
// `text` yang dibalikin di sini SUDAH diubah jadi kalimat temuan (lihat temuanText) — bukan
// teks item mentah lagi, biar langsung kebaca sebagai masalah, bukan pernyataan yang lolos.
export function listFailedItems(record) {
  ...
  cats.forEach((c) => c.items.forEach((text, i) => {
    const key = c.id + "_" + i;
    if (!checks[key]) out.push({ key, text: temuanText(c.id, i, text), rawText: text, catId: c.id, catLabel: c.label });
  }));
  return out;
}
